...
public class WebCheckSteps {
    ...
    public void checkFieldTextByDateToday(String elementName, String currentTodayDate) {
        ...
        LOGGER.info("Дата в поле '{}' совпадает с сегодняшней датой: '{}'", elementName, currentTodayDate);
    }
    ...
    /**
     * Проверка, что значение элемента соответствует регулярному выражению
     *
     * @param elementName - имя элемента
     * @param regex - регулярное выражение
     */
    @Step("элемент {elementName} соответствует регулярному выражению {regex}")
    @Если("элемент {elementName} соответствует регулярному выражению {regex}")
    public void checkFieldTextMatchesRegex(String elementName, String regex){
        SelenideElement element = pageManager
                .getCurrentPage()
                .getElement(elementName);
        Assert.assertTrue(element.getValue().matches(regex));
        LOGGER.info("элемент {} соответствует регулярному выражению {}", elementName, regex);
    }
