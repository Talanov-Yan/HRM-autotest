...
public class WebSteps {
    ...
    public static Properties loadProperties() {
        ...
        try {
            ...
            try {
                inputStream = new FileInputStream("autotest-web/src/main/resources/application.properties");
                properties.load(inputStream);
    ...
    /**
     * Клик по любой ссылке в блоке и сохранение значения выбранного блока в ContextHolder
     *
     * @param fieldName - название блока
     * @param elementName - название значения
     */
    @Step("на текущей странице в блоке {fieldName} нажать на любую ссылку и сохранить значение {elementName} в ContextHolder")
    @И("на текущей странице в блоке {string} нажать на любую ссылку и сохранить значение {string} в ContextHolder")
    public void clickRandomAndSave(String fieldName, String elementName) {
        ElementsCollection elements = pageManager
                .getCurrentPage()
                .getElementsCollection(fieldName);
        int indexElement = Actions.getRandom(elements.size());
        SelenideElement elementKey = elements.get(indexElement);
        String value = pageManager
                .getCurrentPage()
                .getElementsCollection(elementName)
                .get(indexElement)
                .getText();
        ContextHolder.put(fieldName, value);
        elementKey.click();
        LOGGER.info("на странице '{}' выбран элемент '{}'", pageManager.getCurrentPage().name(), fieldName);
    }
